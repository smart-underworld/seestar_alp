...
def render_once(shared_dir, fmt, cat):
    ...
    return write_solve_fits(shared_dir, img, header={"BAYERPAT":"GRBG","RA":p.ra_hours*15.0,"DEC":p.dec_deg})

def main(shared_dir, model, catalog_path, poll=0.25, format_json="frame_format.json"):
    fmt=load_frame_format(format_json, model); cat=_cat.load(catalog_path); last=None
    print(f"[renderd] model={model} fmt={fmt} watching {shared_dir}")
    while True:
        p=read_pointing(shared_dir)
        if p and p.seq!=last:
            print(f"[renderd] pointing.seq={p.seq} -> frame.seq={render_once(shared_dir,fmt,cat)}"); last=p.seq
        time.sleep(poll)

if __name__=="__main__":
    ap=argparse.ArgumentParser()
    ap.add_argument("--shared",default="sim/shared"); ap.add_argument("--model",default="S50")
    ap.add_argument("--catalog",default="sim/data/stars.npy"); a=ap.parse_args()
