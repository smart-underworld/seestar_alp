...
def render_field(cat, ra0_deg, dec0_deg, fmt, mag_zero=12.0, fwhm_px=2.5, bg=1000, noise=True):
    ...
    sigma = fwhm_px/2.3548; half = int(np.ceil(3*sigma))
    for x,y,m in zip(xs,ys,mags):
        ...
        gx=np.arange(x0,x1)-x; gy=(np.arange(y0,y1)-y)[:,None]
        img[y0:y1,x0:x1]+=peak*np.exp(-(gx**2+gy**2)/(2*sigma**2))
    if noise:
        rng=np.random.default_rng(7); img+=rng.normal(0, np.sqrt(np.maximum(img,1))*0.5)
