...
def project(cat_near, ra0_deg, dec0_deg, fmt, roll_deg=0.0):
    ra=np.radians(cat_near[:,0]); dec=np.radians(cat_near[:,1])
    ...
    dx=xi/scale; dy=eta/scale
    r=np.radians(roll_deg)
    xr=dx*np.cos(r)-dy*np.sin(r); yr=dx*np.sin(r)+dy*np.cos(r)
    xs=fmt.width/2+xr; ys=fmt.height/2-yr
