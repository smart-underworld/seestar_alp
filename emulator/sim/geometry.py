...
SENSOR = {
    "S50": (1920, 1080), "S50V2": (1920, 1080), "S30": (1920, 1080),
    "S50P": (3856, 2180), "S30P": (3856, 2180),
}
...
def frame_format(model: str) -> FrameFormat:
    w, h = SENSOR[model]
    return FrameFormat(width=w, height=h, stride_bytes=w * 2,
                       plate_scale_arcsec=_SCALE[model], focal_len_mm=_FOCAL[model])
...
def load_frame_format(path, model: str) -> FrameFormat:
    ...
    if path:
        ...
        fmt = replace(fmt, **{k: v for k, v in over.items()
                              if k in FrameFormat.__dataclass_fields__})
