...
Drives the real seestar_alp application (the classic frontend) against
either the in-repo QEMU emulator (`emulator/`) or a real Seestar, through
actual browser automation (Playwright) — the full app is launched as a
real subprocess (`root_app.py`) and driven by clicking through the actual
rendered pages, not by calling internal functions or HTTP APIs directly.

## What it does

In one continuous session against the chosen target:
...
flow — e.g. before a release, after changing anything in `device/` or
`front/`, or when you suspect a regression that only shows up against the
real protocol (as opposed to the in-repo fake simulator `tests/integration/`
uses).
